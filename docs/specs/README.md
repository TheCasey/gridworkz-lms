# Specs

Use this folder for scoped work that should result in implementation.

Specs can represent:

- incomplete features that need product/technical decisions
- future features that are concrete enough to break into tasks
- UI/UX requirements for a dedicated stream of work

Current specs:

- [core-pricing-and-launch-positioning.md](core-pricing-and-launch-positioning.md)
- [dashboard-shell-and-navigation.md](dashboard-shell-and-navigation.md)
- [homeschool-dashboard.md](homeschool-dashboard.md)
- [weekly-blocking-and-assignment-templates.md](weekly-blocking-and-assignment-templates.md)
- [curriculum-template-and-assignment-model.md](curriculum-template-and-assignment-model.md)
- [weekly-planning-and-review-flow.md](weekly-planning-and-review-flow.md)
- [projects-and-assessment-model.md](projects-and-assessment-model.md)
- [chores-and-rewards-module.md](chores-and-rewards-module.md)
- [reporting-and-compliance-contract.md](reporting-and-compliance-contract.md)
- [ai-assisted-planning-and-student-help.md](ai-assisted-planning-and-student-help.md)
- [lockdown-browser-extension-plan.md](lockdown-browser-extension-plan.md)
- [lockdown-production-behavior-contract.md](lockdown-production-behavior-contract.md)
- [operator-entitlement-console.md](operator-entitlement-console.md)
- [mobile-ui-requirements.md](mobile-ui-requirements.md)
- [report-evidence-drawer.md](report-evidence-drawer.md)
- [public-marketing-site-and-landing-page.md](public-marketing-site-and-landing-page.md)
