# Dashboard Shell And Navigation Workflow

Source plan: `docs/specs/dashboard-shell-and-navigation.md`

## Summary

Rebuild the parent dashboard shell around grouped Homeschool and Chores navigation, a persistent Live Pulse utility rail, dedicated section dashboards, and an Account Settings route without reopening deeper curriculum or chores behavior beyond what the shell needs.

## Roles

- `master-developer`: Own shell-level orchestration, validate phase scope against the shell and homeschool specs, keep route and navigation changes coherent, and manage state transitions plus git checkpoints.
- `developer`: Implement only the active dashboard-shell phase, keep changes narrow, and preserve the current page-owned data-loading pattern unless the shell contract requires a focused exception.
- `tester`: Validate only the active shell phase with route, navigation, layout, and browser-smoke evidence, and confirm any no-test rationale is still sound for the repo's current tooling state.
- `researcher`: Resolve bounded shell, routing, or information-architecture blockers without drifting into deeper curriculum, reports, or chores product redesign.

## Phase Map

| Phase | Downstream Roles | Validation Modes | Depends On | Status |
| --- | --- | --- | --- | --- |
| [Phase 1: Shell Route And Navigation Foundation](phases/phase-01-shell-route-and-navigation-foundation/phase.md) | `developer -> tester` | `build-health`, `browser-smoke` | None | `ready_for_master_developer` |
| [Phase 2: Persistent Live Pulse Utility Rail](phases/phase-02-persistent-live-pulse-utility-rail/phase.md) | `developer -> tester` | `build-health`, `interaction-smoke` | Phase 1: Shell Route And Navigation Foundation | `pending` |
| [Phase 3: Students Overview Refresh](phases/phase-03-students-overview-refresh/phase.md) | `developer -> tester` | `build-health`, `browser-smoke` | Phase 2: Persistent Live Pulse Utility Rail | `pending` |
| [Phase 4: Homeschool Section Dashboard](phases/phase-04-homeschool-section-dashboard/phase.md) | `developer -> tester` | `build-health`, `browser-smoke` | Phase 3: Students Overview Refresh | `pending` |
| [Phase 5: Expandable Chores Section Shell](phases/phase-05-expandable-chores-section-shell/phase.md) | `developer -> tester` | `build-health`, `browser-smoke` | Phase 4: Homeschool Section Dashboard | `pending` |
| [Phase 6: Account Settings And Shell Integration Polish](phases/phase-06-account-settings-and-shell-integration-polish/phase.md) | `developer -> tester` | `build-health`, `browser-smoke`, `manual-qa` | Phase 5: Expandable Chores Section Shell | `pending` |
...
- Use gpt-5.5 high for cross-phase routing or state-model debugging, shell-wide UX coherence issues, or final integration review.
- Use gpt-5.5 xhigh only for master-level planning, workflow recovery, or high-risk acceptance decisions.
...
- Default allowed discovery rule: Start with the listed read-first files, then follow dashboard routes, shell helpers, adjacent section pages, and nearby UI state files only as needed for the active phase.
...
- Initial dirty paths before scaffolding: `ocs/README.md`, `docs/architecture.md`, `docs/features/parent-dashboard.md`, `docs/roadmap.md`, `docs/specs/README.md`, `docs/specs/lockdown-browser-extension-plan.md`, `docs/specs/public-marketing-site-and-landing-page.md`, `docs/support/operator-entitlement-console-runbook.md`, `docs/upgrades/README.md`, `docs/upgrades/curriculum-modal-cleanup.md`, `docs/upgrades/reporting-safety-fixes.md`, `docs/upgrades/shell-and-data-layer.md`, `docs/upgrades/subscriptions-and-entitlements.md`, `extensions/chrome-lockdown-poc/allowlist.html`, `extensions/chrome-lockdown-poc/allowlist.js`, `extensions/chrome-lockdown-poc/background.js`, `extensions/chrome-lockdown-poc/blocked.html`, `extensions/chrome-lockdown-poc/blocked.js`, `extensions/chrome-lockdown-poc/manifest.json`, `extensions/chrome-lockdown-poc/options.html`, `extensions/chrome-lockdown-poc/options.js`, `extensions/chrome-lockdown-poc/policy.js`, `extensions/chrome-lockdown-poc/popup.html`, `extensions/chrome-lockdown-poc/popup.js`, `extensions/chrome-lockdown-poc/youtube-content.js`, `firestore.rules`, `functions/src/index.js`, `src/App.jsx`, `src/components/LockdownPolicyPanel.jsx`, `src/constants/dashboardFeatures.js`, `src/constants/entitlements.js`, `src/constants/schema.js`, `src/firebase/trustedOperations.js`, `src/pages/Lockdown.jsx`, `src/pages/StudentPortal.jsx`, `src/utils/entitlementUtils.js`, `src/utils/lockdownPolicyUtils.js`, `docs/icons/`, `docs/specs/chores-and-rewards-module-workflow/`, `docs/specs/chores-and-rewards-module.md`, `docs/specs/core-pricing-and-launch-positioning.md`, `docs/specs/dashboard-shell-and-navigation.md`, `docs/specs/homeschool-dashboard.md`, `docs/specs/lockdown-production-behavior-contract-workflow/`, `docs/specs/lockdown-production-behavior-contract.md`, `docs/specs/lockdown-simplification-and-extension-launcher-workflow/`, `docs/specs/lockdown-simplification-and-extension-launcher.md`, `docs/specs/weekly-blocking-and-assignment-templates.md`, `docs/support/chores-and-rewards-runbook.md`, `docs/support/lockdown-chrome-web-store-upload-plan.md`, `docs/support/lockdown-support-runbook.md`, `docs/support/pre-merge-readiness-checklist.md`, `docs/upgrades/current-surface-finalization-and-rollout.md`, `extensions/chrome-lockdown-poc/guidance.js`, `functions/src/allowanceUtils.js`, `functions/src/choreTrustedValidators.js`, `functions/src/pointLedgerUtils.js`, `functions/src/rewardRedemptionUtils.js`, `scripts/check-allowance-ledger.mjs`, `scripts/check-chores-availability.mjs`, `scripts/check-chores-entitlements.mjs`, `scripts/check-chores-parent-view-model.mjs`, `scripts/check-chores-rewards-e2e.mjs`, `scripts/check-chores-trusted-contracts.mjs`, `scripts/check-lockdown-derived-policy.mjs`, `scripts/check-lockdown-device-management.mjs`, `scripts/check-lockdown-extension-states.mjs`, `scripts/check-lockdown-parent-view-model.mjs`, `scripts/check-lockdown-policy-states.mjs`, `scripts/check-lockdown-release-package.mjs`, `scripts/check-lockdown-resource-normalization.mjs`, `scripts/check-lockdown-work-launcher.mjs`, `scripts/check-points-ledger.mjs`, `scripts/check-reward-redemptions.mjs`, `scripts/check-student-chores-view.mjs`, `scripts/seed-private-beta-smoke-fixtures.mjs`, `scripts/smoke-private-beta-callables.mjs`, `src/components/StudentRewardStore.jsx`, `src/components/student/`, `src/hooks/useChoreSetup.js`, `src/hooks/useStudentChores.js`, `src/pages/dashboard/ChoresRoute.jsx`, `src/utils/allowanceUtils.js`, `src/utils/choreParentViewModel.js`, `src/utils/choreUtils.js`, `src/utils/rewardUtils.js`, `src/utils/workLauncherUtils.js`

- Branch template: `feat/{plan_slug}`
- Branch in use: `codex/dashboard-shell-and-navigation`
