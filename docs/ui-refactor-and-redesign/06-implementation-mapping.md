...
| `ownpath_weekly_chores.html` | `/dashboard/chores/weekly-chores` | `src/pages/dashboard/ChoresRoute.jsx`, `src/utils/choreUtils.js`, `src/firebase/trustedOperations.js` | Dense shared-pool pass landed with student quota tabs, real availability/pending counts, compact pool rows, modal-style add/edit flow, and a student-filtered approval queue. Entitlement gating and trusted callable review/claim lifecycle remain intact. Prototype-only period navigation is deferred until historical pool state has a real persisted read path. |
| `ownpath_monthly_chores.html` | `/dashboard/chores/monthly-chores` | `src/pages/dashboard/ChoresRoute.jsx`, `src/utils/choreUtils.js`, `src/utils/allowanceUtils.js` | Monthly now uses the same dense shared-pool and review composition as weekly, with monthly quota/progress language and real monthly records. Cooldown, eligibility, archive/edit, and review behavior remain on existing data. Prototype-only month browsing is deferred rather than mocked. |
| `ownpath_allowance_v2.html` | `/dashboard/chores/allowance` | `src/pages/dashboard/ChoresRoute.jsx`, `src/utils/allowanceUtils.js`, `src/firebase/trustedOperations.js` | Current-period allowance console now follows the prototype more closely: contextual period label, compact summary strip, dense student ledger cards, topbar refresh, and settings hidden behind an explicit Edit settings control. Actual money movement remains out of scope; trusted sync and bookkeeping handlers are unchanged. Local visual QA can surface the existing Firebase callable failure when the seeded environment cannot sync, but the ledger recovers and renders. |
