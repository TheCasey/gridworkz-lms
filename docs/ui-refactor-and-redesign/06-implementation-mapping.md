...
| --- | --- | --- | --- |
| `ownpath_student_portal.html` | `/student/:slug` | `src/pages/StudentPortal.jsx`, `src/hooks/useStudentPortalWeeklyPlan.js`, `src/utils/workLauncherUtils.js`, `src/hooks/useStudentChores.js`, `src/components/student/StudentSchoolWorkspace.jsx`, `src/components/student/StudentChoresWorkspaceV2.jsx`, `src/components/student/StudentRewardStoreV2.jsx`, `src/components/student/StudentAvatarWorkspace.jsx` | Approved prototype direction and first live implementation are aligned across School, Chores, Allowance, Rewards, and My Avatar on desktop and mobile. School is weekly-choice based: subject accordions reveal numbered blocks, and selecting a block expands its own subject row with real instructions, resources, response requirements, and existing timer/submission controls. Chores separates Daily Routine, Weekly Chores, and Monthly Chores while preserving trusted completion/claim flows. Rewards preserves trusted request/cancel behavior. Allowance is deliberately Coming Soon; avatar selection is preview-only until a trusted asset-ID persistence contract exists. See `08-student-portal-prototype-mapping.md`. |
...
| `curriculum.html` | `/dashboard/curriculum` | `src/pages/Curriculum.jsx`, `src/components/curriculum/BlockObjectivesEditor.jsx`, `src/hooks/useSubjects.js`, `src/hooks/useSubjectMutations.js` | UI slices landed: prototype-style topbar, student tabs, compact subject tile grid, subject detail/block list view, and usage notice use the redesign language. The old four-step Add/Edit Subject wizard is no longer the normal flow: a compact settings modal asks only for name, block length, written-summary default, optional resources, and an optional multi-student create picker. Create defaults to the active student tab and fans out through the trusted mutation into one independent subject record per selected student. New subjects are empty containers with an explicit `curriculum_blocks: []`; parents open the subject and use Add Block to build the reusable library. Existing multi-student `student_ids`, legacy `student_id`, `block_count`, `block_objectives`, and populated `curriculum_blocks` records remain compatibility-readable without destructive normalization. Subject detail manages reusable block add/edit/remove/pin/type/default-quantity controls, and the focused block editor owns instructions, resources, timer settings, written response, and custom response fields. Legacy `block_objectives` continue to be projected when block definitions change. |
| `weekly-blocking.html` | `/dashboard/weekly-blocking` | `src/pages/dashboard/WeeklyBlockingRoute.jsx`, `src/components/curriculum/WeeklyPlanReviewPanel.jsx`, `src/hooks/useWeeklyPlanRecord.js`, `src/hooks/useWeeklyPlansForWeek.js`, `src/utils/weeklyPlanUtils.js` | First real route landed. It keeps the page name `Weekly Blocking`, uses the prototype-style topbar, student tabs, week chips, subject-row schedule, and summary rail, and supports reset-from-subject-defaults, draft save, and publish through the existing `weeklyPlans` flow. Each subject row shows reusable curriculum block definitions as compact quantity chips, supports per-block add/subtract controls, allows whole-subject enable/disable by zeroing or restoring quantities, and expands repeated selections into unique weekly plan blocks for compatibility with existing student/report completion flows. A single reusable block can therefore be assigned multiple times without duplicate definitions. Subjects with an explicit empty block library show a Needs blocks state and link back to Curriculum instead of generating compatibility blocks or quantity controls. Weekly Blocking intentionally does not expose title/instruction edit fields; detailed block content editing lives in Curriculum. Expanded weekly blocks still carry block-specific resources, timer requirements, written-response requirements, and custom response fields when defined, falling back to subject defaults otherwise. The summary rail can save the current selections back to subject defaults. Saved-plan warnings clarify when subject edits require reset/save/publish. Copy-week, separate assignment-template persistence, and day-level scheduling remain deferred until a schema/rules decision. |
...
| `ownpath_daily_routines_v2.html` | `/dashboard/chores/daily-routines` | `src/pages/dashboard/ChoresRoute.jsx`, `src/hooks/useChoreSetup.js`, `src/utils/choreParentViewModel.js` | The route now treats Morning, Afternoon, and Evening as fixed per-student periods. Parents add or edit one checklist item at a time; optional multi-student selection creates independent per-student copies rather than a shared record. Existing legacy/shared templates remain readable until a child receives a canonical period record, and an empty canonical period intentionally suppresses legacy content after the last item is deleted. Routine allowance and point eligibility are household/day settings: a routine day counts and awards its single configured point event only after every populated period for that student is complete. The prototype student tabs, week navigation, item rows, seven-day cells, and summary rail remain intact; history cells are read-only because no trusted parent-correction callable exists. |
...
| `ownpath_rewards_v3.html` | `/dashboard/chores/rewards` | `src/pages/dashboard/ChoresRoute.jsx`, `src/hooks/useChoreSetup.js`, `src/utils/rewardUtils.js`, `src/components/StudentRewardStore.jsx`, `src/firebase/trustedOperations.js` | Screenshot-reconciliation pass now matches the prototype's three consoles: Reward Store, Students & Points, and Point Settings. The store uses real built-in and parent-created catalogs; the split student view now subscribes to real `pointLedgerEntries` and keeps adjustment and redemption decisions on trusted handlers; settings use the persisted reward-settings contract. Built-in enable/price controls and streak bonuses remain non-editable because those values are not persisted, and weekly/monthly chores correctly share the existing `chore_block_points` setting. |
