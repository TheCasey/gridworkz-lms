...
| --- | --- | --- | --- |
| `ownpath_student_portal.html` | `/student/:slug` | `src/pages/StudentPortal.jsx`, `src/hooks/useStudentPortalWeeklyPlan.js`, `src/utils/workLauncherUtils.js`, `src/hooks/useStudentChores.js`, `src/components/student/StudentChoresWorkspace.jsx`, `src/components/StudentRewardStore.jsx` | Standalone review prototype maps the real portal into School, Chores, Allowance, Rewards, and My Avatar workspaces with desktop and mobile layouts. School is weekly-choice based: subject accordions reveal numbered blocks, and selecting a block opens its instruction/resource/timer panel without day assignments. Chores separates Daily Routine, Weekly Chores, and Monthly Chores. Allowance is deliberately Coming Soon. Existing PIN, published-plan compatibility, block response, trusted chore actions, and trusted reward actions are represented without changing their contracts. See `08-student-portal-prototype-mapping.md`. |
