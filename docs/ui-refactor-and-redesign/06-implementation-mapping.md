...
| --- | --- | --- | --- |
| `ownpath_student_portal.html` | `/student/:slug` | `src/pages/StudentPortal.jsx`, `src/hooks/useStudentPortalWeeklyPlan.js`, `src/utils/workLauncherUtils.js`, `src/hooks/useStudentChores.js`, `src/components/student/StudentChoresWorkspace.jsx`, `src/components/StudentRewardStore.jsx` | Standalone review prototype maps the real portal into School, Chores, Allowance, Rewards, and My Avatar workspaces with desktop and mobile layouts. School is weekly-choice based: subject accordions reveal numbered blocks, and selecting a block expands its own subject row with instructions, resources, response requirements, and timer controls. No day assignments or separate selected-block destination are introduced. Chores separates Daily Routine, Weekly Chores, and Monthly Chores. Allowance is deliberately Coming Soon. Existing trusted behavior remains represented without changing its contracts. See `08-student-portal-prototype-mapping.md`. |
...
| `ownpath_daily_routines_v2.html` | `/dashboard/chores/daily-routines` | `src/pages/dashboard/ChoresRoute.jsx`, `src/hooks/useChoreSetup.js`, `src/utils/choreParentViewModel.js` | Screenshot-reconciliation pass now matches the prototype's dominant structure: real-data student tabs and week navigation, Morning/Afternoon/Evening sections, checklist-item rows, seven-day completion cells, and the fixed weekly summary rail. Existing grouped-checklist persistence and trusted create/edit/archive handlers remain unchanged. Time-of-day uses a compatibility read when present and otherwise derives from the template title; day cells are read-only because no trusted parent correction callable exists. |
...
- Review `ownpath_student_portal.html` on desktop and mobile before replacing the live route.
- Implement the shell around the existing published-plan launcher, timer, submission, chore, and reward contracts.
