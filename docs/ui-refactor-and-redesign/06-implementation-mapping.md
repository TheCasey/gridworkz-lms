...
| --- | --- | --- | --- |
| `ownpath_student_portal.html` | `/student/:slug` | `src/pages/StudentPortal.jsx`, `src/hooks/useStudentPortalWeeklyPlan.js`, `src/utils/workLauncherUtils.js`, `src/hooks/useStudentChores.js`, `src/components/student/StudentChoresWorkspace.jsx`, `src/components/StudentRewardStore.jsx` | Standalone review prototype now maps the real portal into School, Chores, Rewards, and My Avatar workspaces with desktop and mobile layouts. PIN, published-plan compatibility, timers, block response, trusted chore actions, and trusted reward actions are represented without changing their contracts. Avatar persistence, real avatar art, photo evidence upload, and installable-PWA metadata remain implementation decisions. See `08-student-portal-prototype-mapping.md`. |
