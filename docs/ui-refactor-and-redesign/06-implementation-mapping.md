...
| --- | --- | --- | --- |
| `ownpath_students.html` | `/dashboard/students` | `src/pages/dashboard/StudentsRoute.jsx`, `src/components/AddStudentModal.jsx`, `src/components/StudentCard.jsx`, `src/hooks/useStudents.js`, `src/hooks/useStudentMutations.js` | First app slice landed: roster + detail panel, portal link copy, dark modal, fallback `StudentCard`, and 4-6 digit PIN support use the redesign language. Password mode is visible as planned but still requires schema/rules/portal migration before persistence. |
| `ownpath_homeschool_overview.html` | `/dashboard/homeschool` | `src/pages/dashboard/HomeschoolRoute.jsx`, `src/hooks/useWeeklyActivity.js`, `src/hooks/useWeeklyPlansForWeek.js`, `src/hooks/useWeeklyReportRecords.js` | First app slice landed as an overview/launch surface. Links to Curriculum, Weekly Blocking, and Reports. |
| `curriculum.html` | `/dashboard/curriculum` | `src/pages/Curriculum.jsx`, `src/components/curriculum/BlockObjectivesEditor.jsx`, `src/hooks/useSubjects.js`, `src/hooks/useSubjectMutations.js` | UI slices landed: route shell, subject cards, usage notice, modal frame, schedule/resources internals, and `BlockObjectivesEditor` now use the redesign language. New subject creates fan out into per-student subject records through `useSubjectMutations`; existing multi-student `student_ids` and legacy `student_id` records remain readable/editable as compatibility records. Backfill or destructive split behavior for existing shared records is not implemented. The embedded weekly-plan bridge has been replaced by a handoff to the dedicated Weekly Blocking route. Block-objective editing now has a direct card shortcut and normalized save shape for updates. |
| `weekly-blocking.html` | `/dashboard/weekly-blocking` | `src/pages/dashboard/WeeklyBlockingRoute.jsx`, `src/components/curriculum/WeeklyPlanReviewPanel.jsx`, `src/hooks/useWeeklyPlanRecord.js`, `src/hooks/useWeeklyPlansForWeek.js`, `src/utils/weeklyPlanUtils.js` | First real route landed. It keeps the page name `Weekly Blocking`, groups generated weekly plan blocks by subject, supports student/week selection, regenerate-from-subjects, draft save, and publish through the existing `weeklyPlans` flow. Block rows surface objective/field badges, and saved-plan warnings clarify when subject edits require regeneration before save/publish. Reusable default-week templates, copy-week, quantity replacement, per-week objective selection, and assignment-template persistence remain deferred until a schema/rules decision. No day-level scheduling added. |
| `ownpath_chores_overview.html` | `/dashboard/chores` | `src/pages/dashboard/ChoresRoute.jsx`, `src/hooks/useChoreSetup.js`, `src/utils/choreParentViewModel.js`, `src/utils/choreUtils.js`, `src/utils/rewardUtils.js` | First app slice landed for overview framing and launch cards while preserving existing route branching. |
| `ownpath_daily_routines_v2.html` | `/dashboard/chores/daily-routines` | `src/pages/dashboard/ChoresRoute.jsx`, `src/hooks/useChoreSetup.js`, `src/utils/choreParentViewModel.js` | Preserve Free access for daily routines. Daily routines are per-student grouped routines. |
| `ownpath_weekly_chores.html` | `/dashboard/chores/weekly-chores` | `src/pages/dashboard/ChoresRoute.jsx`, `src/utils/choreUtils.js`, `src/firebase/trustedOperations.js` | First visual slice landed for shared pool cards and approval queue. Preserve entitlement gating and trusted callable review/claim lifecycle. |
| `ownpath_monthly_chores.html` | `/dashboard/chores/monthly-chores` | `src/pages/dashboard/ChoresRoute.jsx`, `src/utils/choreUtils.js`, `src/utils/allowanceUtils.js` | First visual slice landed through the same shared weekly/monthly rendering path. Continue refining monthly cadence language in the editor/details as needed. |
| `ownpath_allowance_v2.html` | `/dashboard/chores/allowance` | `src/pages/dashboard/ChoresRoute.jsx`, `src/utils/allowanceUtils.js`, `src/firebase/trustedOperations.js` | First visual passes landed for allowance settings, quota/progress cards, ledger summary, and student bookkeeping cards. Actual money movement remains out of scope; trusted allowance sync/bookkeeping handlers are preserved. |
| `ownpath_rewards_v3.html` | `/dashboard/chores/rewards` | `src/pages/dashboard/ChoresRoute.jsx`, `src/utils/rewardUtils.js`, `src/components/StudentRewardStore.jsx`, `src/firebase/trustedOperations.js` | First visual passes landed for point settings, wallets, reward catalog, reward editor, redemption queue, and built-in placeholder cards. Trusted point/reward handlers and placeholder cosmetic scope are preserved. |
| `ownpath_reports.html` | `/dashboard/reports` | `src/pages/dashboard/ReportsRoute.jsx`, `src/pages/Reports.jsx`, `src/hooks/useWeeklyReportRecords.js`, `src/utils/reportUtils.js` | UI slices landed: shell, filters, live student cards, subject rows, and official record list use the redesign language. Current save, print, delete, manual-save, auto-archive, and filter behavior has been reviewed and preserved. Evidence/readiness now uses an allow-with-warning contract for published weekly-plan snapshots: incomplete assigned blocks and completed required-response blocks missing written detail are surfaced before save, but official record saving is not blocked. |
...
- Weekly Blocking has been converted from placeholder/embedded bridge into an explicit parent route while keeping the current `weeklyPlans` data flow. Subject-derived block objectives flow into regenerated weekly blocks; saved weekly plans intentionally remain stable until regenerated.
...
- Adding persisted Weekly Blocking templates, default-week inheritance, copy-week behavior, or quantity-based block replacement.
