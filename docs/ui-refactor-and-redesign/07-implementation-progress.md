...
- A standalone responsive student portal prototype is ready for review in `ownpath_student_portal.html`. School now follows the weekly-choice model: compact subject accordions reveal numbered blocks, and selecting a block opens its instructions, resources, response requirements, and timer without assigning work to days. Chores is separated into Daily Routine, Weekly Chores, and Monthly Chores. Allowance is a separate Coming Soon workspace while bounty/earning details are finalized. Rewards and layered My Avatar remain included; desktop uses top tabs/right rails and mobile uses a five-item bottom navigation bar. `08-student-portal-prototype-mapping.md` records how each prototype state maps to current portal contracts. This is not yet a replacement of `StudentPortal.jsx`.
