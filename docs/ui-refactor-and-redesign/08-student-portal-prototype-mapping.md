...
| Selected school block | `StudentPortal.jsx`, `timerUtils.js`, `workLauncherUtils.js` | Student-chosen block, start, pause, resume, reset, single-active-timer rules, resources and completion | Tapping a subject expands its weekly blocks; tapping a block updates the focused instruction/resource/timer panel. |
